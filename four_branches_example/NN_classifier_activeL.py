...
for i in range(n_EDini):
    labels[i] = 0 if(LSF(initial_design[i, 0], 
                  initial_design[i,1]) <= 0) else 1  # Evaluate performance function
...
while(1):
    ...
    for j in hidden_layers:
        ...
        model = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, activation='logistic', max_iter = n_epochs ,solver = 'lbfgs')
    ...
    for i in range(n_add):
        labels_best_points[i] = 0 if(LSF(best_points[i][0], best_points[i][1]) <= 0) else 1
