...
for i in range(n_EDini):
    labels[i] = performance_function(DoE[i, 0], DoE[i, 1], DoE[i, 2], DoE[i, 3], DoE[i, 4],
                                     DoE[i, 5])  # Evaluate performance function
...
for _ in range(n_splits):
    model = MLPRegressor(hidden_layer_sizes=(20, 20), max_iter=100000, activation='tanh', solver='lbfgs',
                         early_stopping=True)
    models.append(model)

base_model = MLPRegressor(hidden_layer_sizes=(20, 20), max_iter=100000, activation='tanh', solver='lbfgs',
                          early_stopping=True)
iter = 0
...
while True:
    ...
    label_best_point = np.tanh(
        performance_function(x_best_point[0], x_best_point[1], x_best_point[2], x_best_point[3], x_best_point[4],
                             x_best_point[5]))
